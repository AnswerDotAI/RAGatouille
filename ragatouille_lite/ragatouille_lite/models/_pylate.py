...
class PyLateColBERT(LateInteractionModel):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        if n_gpu == -1:
            self.n_gpu = (
                1 if torch.cuda.device_count() == 0 else torch.cuda.device_count()
            )
        ...
        if load_from_index:
            self._init_from_index(pretrained_model_name_or_path)
        else:
            self._init_from_pretrained(pretrained_model_name_or_path)

    def _init_from_index(self, index_path: str):
        # self.index_path = str(pretrained_model_name_or_path)
        #     ckpt_config = ColBERTConfig.load_from_index(
        #         str(pretrained_model_name_or_path)
        #     )
        #     self.model_index = ModelIndex.load(
        #         self.index_path, index_name, ckpt_config, verbose=self.verbose
        #     )
    ...
    def _init_from_pretrained(self, pretrained_model_name_or_path: str):
        pass
    ...
    def add_to_index(
        ...
        if self.loaded_from_index and not self.collection:
            collection_path = (
                Path(self.config.index_root) / self.index_name / "collection.json"
            )
            if collection_path.exists():
                self._get_collection_files_from_disk(
                    str(Path(self.config.index_root) / self.index_name)
                )
    ...
    def delete_from_index(
        ...
        # TODO: Moving to indexing/metadata.py
        pass
    ...
    def index(
        ...
        if model_type == "PLAID":
            self.model_index = ModelIndex.construct(
                model_type,
                self.config,
                self.checkpoint,
                self.collection,
    ...
    def encode_queries(
        ...
        if isinstance(queries, str):
            queries = [queries]
        maxlen = max([int(len(x.split(" ")) * 1.4) for x in queries])
        maxlen = int(((maxlen + 31) // 32) * 32)

        embedded_queries = self.model.encode(
            queries,
            is_query=True,
        )
    ...
    def encode_documents(
        ...
        embedded_docs = self.model.encode(
            documents,
            is_query=False,
        )
    ...
    def rank(
        ...
        queries_embeddings = self.encode_queries(query, bsize=bsize)
        documents_embeddings = self.encode_documents(documents, bsize=bsize)
        if document_ids is None:
            document_ids = [str(i) for i in range(len(documents))]
        # TODO: check output format
        reranked_documents = pylate_rank.rerank(
