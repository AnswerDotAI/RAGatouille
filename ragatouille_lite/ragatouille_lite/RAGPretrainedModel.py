...
from ragatouille_lite.data.corpus_processor import CorpusProcessor
from ragatouille_lite.data.preprocessors import simple_sentence_splitter
from ragatouille_lite.integrations import (
    RAGatouilleLangChainCompressor,
    RAGatouilleLangChainRetriever,
)
from ragatouille_lite.models import (
    TransformersColBERT,
    LateInteractionModel,
)

try:
    from ragatouille.models import StanfordColBERT
...
class RAGPretrainedModel:
    ...
    def from_pretrained(
        ...
        backend: Literal["Transformers", "Stanford"] = "Transformers",
    ):
        ...
        if backend == "Stanford":
            if RAGATOUILLE_AVAILABLE:
                raise NotImplementedError(
                    "Lite does not yet support multiple backends."
                )
                instance.model = StanfordColBERT(
                ...
                raise ImportError(
                    "You must install the full `ragatouille` package to use StanfordColBERT!"
                )
        elif backend == "Transformers":
            instance.model = TransformersColBERT(
                "pretrained_model_name_or_path", index_root=index_root, verbose=verbose
            )
        return instance
